...
class DefaultDaemon(Daemon):
    def run(self):
        logger.info('Starting CM Service')
...
if __name__ == "__main__":
    ...
    handler = logging.FileHandler(os.path.join(pwd, 'cm-service.log'))
    ...
    daemon = DefaultDaemon(os.path.join(pwd, 'cm-service.pid'))
    if len(sys.argv) == 2:
        if 'start' == sys.argv[1]:
            daemon.start()
